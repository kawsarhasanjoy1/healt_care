...
const createBloodDonate = async (payload, user) => {
    ...
    if (!patientInfo.isDonor) {
        throw new AppError(StatusCodes.FORBIDDEN, "You are not registered as a donor");
    }
    ...
    if (lastDonationRecord) {
        ...
        if (diffInDays < minimumGapDays) {
            ...
            throw new AppError(StatusCodes.BAD_REQUEST, `আপনি এখন রক্ত দিতে পারবেন না। আপনাকে আরও ${remainingDays} দিন অপেক্ষা করতে হবে।`);
        }
    }
    const bloodData = {
        donorId: patientInfo?.id,
        bloodGroup: payload.bloodGroup || patientInfo?.patientHealthData?.bloodGroup,
...
const getAvailableDonorsForPatient = async (user) => {
    const patientInfo = await prisma.patient.findUnique({
        where: { email: user?.email },
        include: {
    ...
    if (!patientInfo.isNeedBlood) {
        throw new AppError(StatusCodes.FORBIDDEN, "দুঃখিত, ডাক্তারের অনুমতি ছাড়া আপনি এই তথ্যগুলো দেখতে বা প্রদান করতে পারবেন না।");
    }
    ...
    const sortedHistory = donationHistory.sort((a, b) => {
        ...
        const bMatchAddress = b.address
            .toLowerCase()
            .includes(patientAddress.toLowerCase());
        if (aMatchAddress && !bMatchAddress)
            return -1;
        if (!aMatchAddress && bMatchAddress)
            return 1;
        return 0;
...
const myBloodDonation = async (user) => {
    const patientInfo = await prisma.patient.findUnique({
        where: { email: user?.email },
        include: {
            patientHealthData: true,
    ...
    if (!patientInfo.isDonor) {
        throw new AppError(StatusCodes.FORBIDDEN, "দুঃখিত, এই তথ্যগুলো দেখতে বা প্রদান করতে পারবেন না।");
    }
