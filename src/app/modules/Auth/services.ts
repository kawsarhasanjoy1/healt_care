...
const loginUser = async (payload: TLogin) => {
  ...
  const matchPass: boolean = await bcrypt.compare(
    password,
  ...
  const accessToken = await createToken(
    ...
    config.access_expirein as string
  );
...
const changePassword = async (
  ...
  const updatePassword = await prisma.users.update({
    ...
    data: { password: hashPass, needPasswordCng: false },
  });
...
const forgatePassword = async (email: string) => {
  ...
  const token = await createToken(
    userData,
    config.forgot_secret as string,
    config.forgot_expireIn
  );

  const resetLink = `${config.front_end_url}/reset-password?userId=${isExistUser?.id}&token=${token}`;
...
const resetPassword = async (
  ...
  let validToken;
  try {
    validToken = verifyToken(token, config.forgot_secret as Secret) as JwtPayload;
  } catch (err) {
    throw new AppError(StatusCodes.UNAUTHORIZED, "You are not authorized");
  }
  if (validToken?.id !== payload?.id) {
    throw new AppError(StatusCodes.FORBIDDEN, "You are not authorized to change this password!");
  }
