...
const createBloodRequest = async (
  payload: { donorId: string; bloodDonateId: string },
  user: TAuthUser
) => {
  ...
  if (!donorInfo?.isDonor) {
    throw new AppError(
      ...
      "প্যাসেন্ট আইডি দিয়ে কোন রক্তদাতা পাওয়া যাচ্ছে না"
    );
  }
  ...
  if (!patient?.isNeedBlood) {
    throw new AppError(
      ...
      "রক্তের অনুরুধের জন্য আপনি ডাক্তার অনুমুধিত না"
    );
  }
...
const getMyRequest = async (user: TAuthUser) => {
  ...
  if (!patiantInfo?.isNeedBlood) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "দুঃখিত, বর্তমানে আপনার রক্ত প্রয়োজন (Need Blood) স্ট্যাটাসটি সক্রিয় নেই। তাই আপনি ডোনারদের তথ্য দেখতে পারবেন না।"
    );
  }
...
const upBloodRequestStatus = async (
  requestId: string,
  ...
  if (request.donorId !== donor.id) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "আপনি এই অনুরোধটি এক্সেপ্ট করার অনুমতিপ্রাপ্ত নন।"
    );
  }
...
const incomingBloodRequest = async (user: TAuthUser) => {
  ...
  if (!donorInfo.isDonor) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      "দুঃখিত, আপনি একজন রক্তদাতা (Donor) হিসেবে নিবন্ধিত নন।"
    );
  }
